...
class Sectioned:
    ...
    def read(
        text: str,
        filter_: _t.Optional[Callable[[str], object]] = None,
    ) -> Generator[Pair]:
...
class EntryPoint:
    ...
    dist: _t.Optional[Distribution] = None
    ...
    def _for(self, dist: Distribution) -> _t.Self:
    ...
    @staticmethod
    def _disallow_dist(params: dict[str, object]) -> None:
        """
    ...
    def __setattr__(self, name: str, value: _t.Any, /) -> _t.NoReturn:
...
class EntryPoints(tuple[EntryPoint, ...]):
    ...
    @classmethod
    def _from_text_for(cls, text: _t.Optional[str], dist: Distribution) -> _t.Self:
        return cls(ep._for(dist) for ep in cls._from_text(text))

    @staticmethod
    def _from_text(text: _t.Optional[str]) -> Generator[EntryPoint]:
        return (
...
class Distribution(metaclass=abc.ABCMeta):
    ...
    def files(self) -> _t.Optional[list[PackagePath]]:
        ...
        def make_file(
            name: str,
            hash: _t.Optional[str] = None,
            size_str: _t.Optional[str] = None,
        ):
        ...
        def make_files(lines: Iterable[str]) -> Iterable[PackagePath]:
        ...
        def skip_missing_files(
            package_paths: Iterable[PackagePath],
        ) -> list[PackagePath]:
        ...
        lines_found = file_lines is not None
        return skip_missing_files(make_files(file_lines)) if lines_found else None
    ...
    def _read_egg_info_reqs(self):
        ...
        return (
            list(self._deps_from_requires_text(source))
...
class DistributionFinder(MetaPathFinder):
    ...
    class Context:
        ...
        def __init__(self, **kwargs: _t.Any):
...
class FastPath:
    ...
    def __init__(self, root: str):
        self.root = root
        self.lookup = functools.lru_cache(self._lookup)
    ...
    def zip_children(self):
        ...
        names: list[str] = zip_path.root.namelist()
...
class MetadataPathFinder(DistributionFinder):
    ...
    @classmethod
    def find_spec(cls, *args: _t.Any, **kwargs: _t.Any):
        """
        A null finder method so that MetadataPathFinder qualifies as a meta path finder,
        but can defer spec finding to the rest of sys.meta_path.
        """
        return None
    ...
    @classmethod
    def _search_paths(cls, name: str, paths):
...
class PathDistribution(Distribution):
    ...
    def read_text(self, filename: str | os.PathLike[str]) -> _t.Optional[str]:
        with suppress(
            FileNotFoundError,
            IsADirectoryError,
            KeyError,
    ...
    def _normalized_name(self):
        ...
        return super()._normalized_name

    @staticmethod
    def _name_from_stem(stem: str):
        """
...
def _unique(dists: Iterable[Distribution]) -> Generator[Distribution]:
    """
    Wrapper for ``distributions`` to return unique distributions by name.
    """
    seen: set[str | None] = set()
    for dist in dists:
        k = py39.normalized_name(dist)
        if k not in seen:
            seen.add(k)
            yield dist
...
def _get_toplevel_name(name: PackagePath) -> str:
    ...
    'foo.dist-info'
    """
    return _topmost(name) or _get_module_name(name) or str(name)


def _get_module_name(path: _t.Union[str, os.PathLike[str]]):
    """Vendored version of inspect.getmodulename() to avoid an expensive import.

    Orignal docstring below:
...
def _top_level_inferred(dist: Distribution):
    dist_files = [] if (dist.files is None) else dist.files
    opt_names = set(map(_get_toplevel_name, dist_files))
