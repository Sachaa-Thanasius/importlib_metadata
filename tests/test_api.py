...
class APITests(
    ...
    def test_prefix_not_matched(self):
        ...
        for prefix in prefixes:
            with self.subTest(prefix), pytest.raises(PackageNotFoundError):
                distribution(prefix)
    ...
    def test_for_top_level(self):
        ...
        for pkg_name, expect_content in tests:
            with self.subTest(pkg_name):
                assert distribution(pkg_name).read_text("top_level.txt").strip() == expect_content
    ...
    def test_read_text(self):
        ...
        for pkg_name, expect_content in tests:
            with self.subTest(pkg_name):
                top_level = next(path for path in files(pkg_name) if path.name == "top_level.txt")
                assert top_level.read_text() == expect_content
    ...
    def test_entry_points_distribution(self):
        ...
        for entry in ("main", "ns:sub"):
            ep = entries[entry]
            assert ep.dist.name in ("distinfo-pkg", "egginfo-pkg")
    ...
    def test_file_hash_repr(self):
        ...
        assert re.search("<FileHash mode: sha256 value: .*>", repr(util.hash))
    ...
    def test_version_egg_info_file(self):
        assert version("egginfo-file") == "0.1"
