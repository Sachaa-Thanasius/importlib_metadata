...
class BasicTests(fixtures.DistInfoPkg, unittest.TestCase):
    ...
    def test_package_not_found_mentions_metadata(self):
        ...
        """
        with pytest.raises(PackageNotFoundError) as ctx:
            Distribution.from_name("does-not-exist")

        assert "metadata" in str(ctx.value)
...
class ImportTests(fixtures.DistInfoPkg, unittest.TestCase):
    ...
    def test_resolve(self):
        ...
        assert ep.load().__name__ == "main"

    def test_entrypoint_with_colon_in_name(self):
        ep = entry_points(group="entries")["ns:sub"]
        assert ep.value == "mod:main"
...
class NonASCIITests(fixtures.OnSysPath, fixtures.SiteDir, unittest.TestCase):
    @staticmethod
    def pkg_with_non_ascii_description(site_dir) -> str:
        """
    ...
    @staticmethod
    def pkg_with_non_ascii_description_egg_info(site_dir) -> str:
        """
...
class DirectoryTest(fixtures.OnSysPath, fixtures.SiteDir, unittest.TestCase):
    ...
    def test_egg(self):
        ...
        egg.mkdir()
        with self.add_sys_path(egg), pytest.raises(PackageNotFoundError):
            version("foo")
...
class FileSystem(fixtures.OnSysPath, fixtures.SiteDir, fixtures.FileBuilder, unittest.TestCase):
...
class PackagesDistributionsTest(fixtures.OnSysPath, fixtures.SiteDir, unittest.TestCase):
...
class PackagesDistributionsEggTest(
    ...
    def test_packages_distributions_on_eggs(self):
        ...
        def import_names_from_package(package_name):
            return {
                import_name for import_name, package_names in distributions.items() if package_name in package_names
            }
